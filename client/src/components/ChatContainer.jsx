    ...
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
      ...
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
        {messages.map((msg, index) => (
          ...
          >
            <div className='flex flex-col max-w-[230px] mb-8'>
              {msg.image && (
                <img
                  src={msg.image}
                  alt='Sent'
                  className='w-full border border-gray-700 rounded-lg overflow-hidden mb-1'
                />
              )}
              {msg.text && (
                <p
                  className={`p-2 text-sm font-light rounded-lg break-all bg-violet-500/30 text-white ${
      ...
      <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3 bg-black'>
        ...
        <img onClick={handleSendMessage} src={assets.send_button} alt='Send' className='w-7 mr-2 cursor-pointer' />
      </div>
    </div>
  );
};
