...
const ProfilePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    reader.readAsDataURL(selectedImg); // ✅ fixed typo (was readeAsDataURL)
    reader.onload = async () => {
      const base64Image = reader.result;
      await updateProfile({ profilePic: base64Image, fullName: name, bio }); // ✅ fixed typo: profielPic → profilePic
      navigate('/');
    };
  };
        ...
        <form onSubmit={handleSubmit} className='flex flex-col gap-5 p-10 flex-1'>
          ...
          <label htmlFor="avatar" className='flex items-center gap-3 cursor-pointer'>
            ...
            <img 
              ...
              alt="Profile Preview" 
              className={`w-12 h-12 ${selectedImg} &&  'rounded-fulll'}`}
            />
